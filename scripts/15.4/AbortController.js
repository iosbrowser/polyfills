// https://www.npmjs.com/package/abortcontroller-polyfill/v/1.7.8 & GPT-5
...
(function (factory) {
            ...
            [
                    ...
                    value: function timeout(time) {
                        ...
                        setTimeout(function () {
                            return controller.abort(
                                new DOMException(
                                    "This signal is timeout in ".concat(
                                        time,
                                        "ms"
                                    ),
                                    "TimeoutError"
                ...
                },
                {
                    ...
                    value: function any(iterable) {
                        ...
                        function clean() {
                            var _iterator =
                                    _createForOfIteratorHelper(iterable),
                                _step;
                            try {
                                for (
                                    _iterator.s();
    ...
    function polyfillNeeded(self) {
        try {
            // Prefer passed-in targets (from abortableFetchDecorator call)
            var AC = self.AbortController || (typeof AbortController !== "undefined" && AbortController);
            var AS = self.AbortSignal || (typeof AbortSignal !== "undefined" && AbortSignal);
            var F = self.fetch || (typeof fetch !== "undefined" && fetch);
            var R = self.Request || (F && F.Request) || (typeof Request !== "undefined" && Request);

            // If AbortController/AbortSignal are missing, we need the polyfill.
            if (!AC || !AS) return true;

            // If there's no fetch, don't install (nowhere to patch); treat as not needed here.
            if (!F) return false;

            // If Request is missing, we still need to patch fetch to honor signals.
            if (!R) return true;

            // Check if Request supports the 'signal' property and constructor init option.
            try {
                var ctrl = new AC();
                var req = new R("about:blank", { signal: ctrl.signal });
                if (!("signal" in req)) return true;
            } catch (e) {
                // Creating a Request with a signal threw => needs polyfill.
                return true;
            }

            // Passed all checks: no polyfill needed.
            return false;
        } catch (_) {
            return true;
        }
    ...
    function abortableFetchDecorator(patchTargets) {
        ...
        if (NativeRequest && !("signal" in (NativeRequest.prototype || {}))) {
            Request = function Request(input, init) {
                var signal;
                if (init && init.signal) {
                    signal = init.signal;
                    // Do not pass init.signal to native Request when polyfilling
                    delete init.signal;
                }
                var request = new NativeRequest(input, init);
                if (signal) {
                    try { request.signal = signal; } catch (e) {}
                }
                return request;
            };
        ...
        var abortableFetch = function abortableFetch(input, init) {
            var signal =
                Request && Request.prototype && Request.prototype.isPrototypeOf(input)
                    ? input.signal
                    : init
                    ? init.signal
                    : undefined;
            if (signal) {
                ...
                if (init && init.signal) {
                    ...
                    // been installed because native may not accept non-native signals.
                    delete init.signal;
                }
    ...
    // Granular patch: add AbortSignal.prototype.throwIfAborted when missing (Safari/iOS < 15.4)
    function patchAbortSignalThrowIfAborted(self) {
        try {
            var AS = self.AbortSignal || (typeof AbortSignal !== "undefined" && AbortSignal);
            if (!AS) return;
            var proto = AS.prototype || {};
    ...
    (function (self) {
        // Always patch missing granular features on native implementations first.
        patchAbortSignalThrowIfAborted(self);
