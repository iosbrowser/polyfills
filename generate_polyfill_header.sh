...

{
  ...
  echo ""
  echo -n "static NSString *$VARIABLE_NAME = @\"console.log('Start $HEADER_FILE');"
} > "$OUTPUT_H_FILE"
...
if [ -s "$TEMP_JS_CONCAT" ]; then
  uglifyjs "$TEMP_JS_CONCAT" -o "$TEMP_JS_CONCAT"
  echo "\\" >> "$OUTPUT_H_FILE"
  sed -e 's/\\/\\\\/g' -e 's/"/\\"/g' -e 's/$/\\/' "$TEMP_JS_CONCAT" >> "$OUTPUT_H_FILE"
  truncate -s-1 "$OUTPUT_H_FILE"
  echo "\";" >> "$OUTPUT_H_FILE"
else
  echo "\";" >> "$OUTPUT_H_FILE"
fi
